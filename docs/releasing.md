...
1. Choose a stable semver tag such as `v0.1.0`.
2. Ensure that tag exactly matches `project.version` after removing the optional leading `v`.
3. Push the release commit and create a GitHub Release with that stable tag.
...
- The publish workflow rejects prereleases.
