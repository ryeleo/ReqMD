...
class TestFooterLegendRegion:
    ...
    def test_RQMD_ui_006_footer_legend_parameter_accepted(self):
        ...
        with patch("rqmd.menus.click.echo", side_effect=capture_echo):
            with patch("sys.stdout.isatty", return_value=False):
                with patch("click.getchar", return_value="q"):
                    try:
                        menus_mod.select_from_menu(
                            "Test", options, footer_legend=custom_legend
                        )
                    except:
                        pass
        ...
        output = "".join(echo_calls)
        assert (
            custom_legend in output
        ), f"Custom legend not found in output. Got: {echo_calls}"
    ...
    def test_RQMD_ui_006_default_legend_when_none_provided(self):
        ...
        assert (
            "keys:" in output.lower() or "quit" in output.lower()
        ), "Default legend should be generated"
                ...
                with patch("click.getchar", return_value="q"):
                    try:
                        ...
                        result = menus_mod.select_from_menu(
                            "Test", options, footer_legend="keys: q=quit"
                        )
        ...
        with patch("rqmd.menus.click.echo"):
            with patch("sys.stdout.isatty", return_value=False):
                with patch("click.getchar", side_effect=["n", "q"]):
                    try:
                        result = menus_mod.select_from_menu(
                            "Menu", options, allow_paging_nav=True, footer_legend=legend
                        )
                        # Should exit cleanly
    ...
    def test_RQMD_ui_006_legend_with_special_characters(self):
        ...
        with patch("rqmd.menus.click.echo", side_effect=capture_echo):
            with patch("sys.stdout.isatty", return_value=False):
                with patch("click.getchar", return_value="q"):
                    try:
                        menus_mod.select_from_menu(
                            "Test", options, footer_legend=complex_legend
                        )
                    except:
                        pass
        ...
        output = "".join(echo_calls)
        assert (
            complex_legend in output
        ), "Special characters in legend should be preserved"
        ...
        with patch("rqmd.menus.click.echo", side_effect=capture_echo):
            with patch("sys.stdout.isatty", return_value=False):
                with patch("click.getchar", return_value="q"):
                    try:
                        menus_mod.select_from_menu(
                            "Test", options, footer_legend=very_long_legend
                        )
                    except:
                        pass
...
class TestFooterMessageTransient:
    ...
    def test_RQMD_ui_006_footer_region_reserved_layout(self):
        ...
        assert (
            "Option 1" in output or "Option 2" in output
        ), "Menu content should render"
        ...
        with patch("rqmd.menus.click.echo", side_effect=capture_echo_2):
            with patch("sys.stdout.isatty", return_value=False):
                with patch("click.getchar", return_value="q"):
                    try:
                        menus_mod.select_from_menu(
                            "Menu", options, footer_legend="Footer"
                        )
                    except:
                        pass

        # Menu items should appear at same line in both cases
        content_lines_no_footer = [
            e for e in echo_calls_no_footer if "Stable" in e or "Content" in e
        ]
        content_lines_with_footer = [
            e for e in echo_calls_with_footer if "Stable" in e or "Content" in e
        ]

        # Both should have menu items rendered
        assert (
            len(content_lines_no_footer) > 0
        ), "Should have content lines without footer"
        assert (
            len(content_lines_with_footer) > 0
        ), "Should have content lines with footer"
        ...
        with patch("rqmd.menus.click.echo"):
            with patch("sys.stdout.isatty", return_value=False):
                with patch("click.getchar", return_value="q"):
                    try:
                        menus_mod.select_from_menu(
                            "Menu", options, zebra=True, footer_legend="keys: q=quit"
                        )
                    except:
                        pass
