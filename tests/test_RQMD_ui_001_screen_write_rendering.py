...
class TestScreenWriteFullScreenRedraw:
    ...
    def _capture_menu_output(
        self, options, screen_write_enabled=False, is_tty=True, user_input="q"
    ):
                ...
                with patch("click.getchar", return_value=user_input):
                    try:
                        menus_mod.select_from_menu(
                            "Test Menu", options, allow_paging_nav=False
                        )
    ...
    def test_RQMD_ui_001_screen_write_enabled_sends_clear_home_escape(self):
        ...
        output = self._capture_menu_output(
            options, screen_write_enabled=True, is_tty=True, user_input="q"
        )

        # Should contain clear (CSI 2J) + home (CSI H) escape
        assert (
            "\x1b[2J\x1b[H" in output
        ), "Expected clear + home cursor escape in TTY screen-write mode"
    ...
    def test_RQMD_ui_001_screen_write_disabled_no_clear_escape(self):
        ...
        options = ["Option 1", "Option 2", "Option 3"]
        output = self._capture_menu_output(
            options, screen_write_enabled=False, is_tty=True, user_input="q"
        )

        # Should NOT contain clear escape when disabled
        assert (
            "\x1b[2J\x1b[H" not in output
        ), "Expected no clear escape when screen-write disabled"
    ...
    def test_RQMD_ui_001_non_tty_no_clear_even_when_enabled(self):
        ...
        options = ["Option 1", "Option 2", "Option 3"]
        output = self._capture_menu_output(
            options, screen_write_enabled=True, is_tty=False, user_input="q"
        )

        # Should NOT send clear for non-TTY (e.g., piped output, file, CI)
        assert (
            "\x1b[2J\x1b[H" not in output
        ), "Expected no ANSI escape for non-TTY even with screen-write enabled"
    ...
    def test_RQMD_ui_001_multiple_renders_with_screen_write(self):
        ...
        output1 = self._capture_menu_output(
            options1, screen_write_enabled=True, is_tty=True, user_input="q"
        )
        ...
        output2 = self._capture_menu_output(
            options2, screen_write_enabled=False, is_tty=True, user_input="q"
        )
        assert (
            "\x1b[2J\x1b[H" not in output2
        ), "Second call should not have clear escape when disabled"
    ...
    def test_RQMD_ui_001_clear_home_order_correct(self):
        ...
        if clear_pos != -1 and home_pos != -1:
            ...
            assert (
                clear_pos < home_pos
            ), "Clear escape should come before home cursor escape"
        else:
            pytest.fail(
                f"Expected both clear and home escapes in output. Clear: {clear_pos}, Home: {home_pos}"
            )
    ...
    def test_RQMD_ui_001_forced_screen_write_overrides_append_mode(self):
        ...
        with patch("sys.stdout", output_buffer):
            with patch("sys.stdout.isatty", return_value=True):
                with patch("click.getchar", return_value="q"):
                    menus_mod.select_from_menu(
                        "Forced", options, allow_paging_nav=False
                    )
