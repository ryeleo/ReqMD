...
class TestNonTTYFallback:
    ...
    def test_RQMD_ui_003_screen_write_disabled_when_not_tty(self):
        ...
        menus_mod.set_screen_write_enabled(
            True
        )  # Enabled, but should be ignored for non-TTY
        ...
        assert (
            "Option A" in output
        ), "Menu content should be present in non-TTY fallback"
    ...
    def test_RQMD_ui_003_ci_environment_no_ansi_codes(self):
        ...
        assert (
            len(screen_write_escapes) == 0
        ), f"CI output should not have clear/home escapes: {ansi_escapes_found}"
    ...
    def test_RQMD_ui_003_module_state_respects_tty_check(self):
        ...
        ), "Non-TTY should not output clear escape even with module state enabled"
                    ...
                    with patch("click.getchar", return_value="q"):
                        try:
                            result = menus_mod.select_from_menu(
                                "Piped Menu", options, allow_paging_nav=False
                            )
                        except:
                            result = None
