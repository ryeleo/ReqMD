...
def extract_domain_summaries(
    ...
    for path in iter_domain_files(repo_root, criteria_dir):
        ...
        status_order = [
            (full_label, full_label.split()[0]) for full_label, _ in STATUS_ORDER
        ]
...
def generate_readme_section(summaries: list[DomainSummary]) -> str:
    ...
    for summary in summaries:
        ...
        lines.append(
            f"- **{summary.display_name}** ({relative_path.name}): {summary.emoji_label}"
        )
...
def sync_readme_from_domains(
    repo_root: Path, criteria_dir: str = "docs/requirements"
) -> tuple[bool, str]:
