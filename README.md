...
Bundle bootstrap can also be driven through a structured chat-style preview with `uv run rqmd-ai install --json --bundle-preset minimal --bootstrap-chat --dry-run`. That payload includes inferred interview questions, detected command sources, and preview content for the generated `/dev` and `/test` skills; follow up with `--bootstrap-answer FIELD=VALUE` overrides before writing when the inferred commands need correction.
